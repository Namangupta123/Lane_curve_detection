...
def canny(image):
    gray = cv2.cvtColor(image,
                        cv2.COLOR_RGB2GRAY)  #it converts our image in a gray because it has only 1 channel means 1 pixel it is more faster
...
def region_of_interest(image):
    ...
    masked_image = cv2.bitwise_and(image, mask)  #masking the canny img to only show region of interest
    return masked_image
